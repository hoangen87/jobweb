...
export default function JobDescriptionManager({ jobDescriptions }: { jobDescriptions: JobDescription[] }) {
  ...
  async function upload(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Không thể tải JD.");
      setOpen(false);
      router.refresh();
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : "Không thể tải JD.");
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <div>
      ...
      {open && (
        <div className="absolute right-0 top-full z-40 mt-2 w-[min(720px,calc(100vw-2rem))] rounded-xl border border-brand-200 bg-white p-5 shadow-xl">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-gray-900">Thư viện JD</h2>
          ...
          <form onSubmit={upload} className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            >
              <option value="">Tạo JD mới</option>
            ...
            <input name="title" required className="input-field" placeholder="Tên JD / vị trí tuyển dụng" />
            ...
            <button type="submit" disabled={uploading} className="btn-primary">
              {uploading ? "Đang tải..." : replaceId ? "Cập nhật phiên bản JD" : "Lưu JD mới"}
            </button>
          </form>
          <p className="mt-2 text-xs text-gray-500">Hỗ trợ PDF, DOCX, TXT; tối đa 10 MB.</p>
          {error && <p className="mt-3 text-sm text-red-600">{error}</p>}
          {jobDescriptions.length > 0 && (
            <div className="mt-4 max-h-64 divide-y overflow-y-auto rounded-lg border">
              {jobDescriptions.map((jd) => (
                <div key={jd.id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                  ...
                  <a href={jd.filePath} target="_blank" rel="noreferrer" className="text-brand-600 hover:underline">
                    Xem JD
                  </a>
                </div>
              ))}
