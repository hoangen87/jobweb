...
export async function GET() { if (!(await authorized())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 }); return NextResponse.json({ ...(await getGeminiConfiguration()), provider: GEMINI_PROVIDER, baseUrl: GEMINI_BASE_URL, task: "Đánh giá hồ sơ ứng viên" }); }
